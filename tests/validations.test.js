...
test('checks if a malformed URL is a valid one', () => {
    expect(urlValidation("//interclip.app/")).toBe("This doesn't seem to be a valid URL");
});
...
test('checks if a string with # is valid', () => {
    expect(validationMsg("yes#f")).toBe("There are some characters, that shouldn't be there.");
});
