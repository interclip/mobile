...
const urlValidation = (url) => {
  ...
  if (url.length === 0)
    return "Start pasting or typing in the URL";
...
export function HomeScreen({navigation}) {
  ...
  color = "#000"        // White color for contrast on the Header
              /
          > </TouchableOpacity>
      </Header><View>< TouchableOpacity
  onPress = {() => navigation.navigate("Send")} > < Image
  style = {styles.previewImg} source = {
    {
  ...
  maxLength = {config.codeMaxLength} inputStyle =
  {
    ...
    () => { Linking.openURL(data); }}
              style={{
    color: checkError(data) ? colors.light
...
export function QRScreen({navigation}) {
  ...
  const handleBarCodeScanned = ({_type, data}) => {
    ...
    if (result === "https://iclip.netlify.com" |
        result === "http://iclip.netlify.app" | Settings.get("data")) {
      Vibration.vibrate();
      data.indexOf("http") > -1
          ? Linking.openURL(data)
...
export function SettingsPage() {
  ...
  return (
    ...
    ...styles.container,
        backgroundColor: colorScheme === "dark" ? "#333333" : "#f4f4f4" }}>
      <Text style={{
    color: colorScheme === "dark" ? "white" : "black" }}>Open all QR Codes automatically</Text>
      <Switch
...
export function SendScreen( {navigation} ) {
 ...
 keyboardType = {Platform.OS === "android" ? "default" : "url"} style = {
     styles.container} placeholder = "Your URL here"
 inputStyle =
 {
   { fontSize: 25 }
 } autoCorrect = {false} autoCompleteType = {"off"} returnKeyType =
     {Platform.OS === "android" ? "none" : "done"} onChangeText = {
         (text) => setText(text)} defaultValue = {text} errorStyle = {
       { color: "red" }
     } autoCapitalize = "none"
         autoFocus={true}
    ...
    () => { !isLoading && Linking.openURL(data) }}
       ...
       <View style={{ padding: 24, flexDirection:"row" }}>
         {isLoading ? (
           ...
           <Text
         onPress = { () => { Linking.openURL(data); } } style =
             {
               {
                 color: checkError(data) ? colors.light : colors.text,
                     backgroundColor: checkError(data)&!urlValidation(text)
                         ? colors.errorColor
                         : null,
                     fontSize: 40, marginLeft: "20%"
               }
             } > {!urlValidation(text) &&
